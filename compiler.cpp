...
// тип указателя на функцию парсинга
...
static Token syntheticToken(const char *text) {
    Token token;
    token.start = text;
    token.length = static_cast<int>(std::strlen(text));
    return token;
}
...
static void function(FunctionType type) {
    ...
    ObjFunction *functionNode = endCompiler();
    emitBytes(OP_CLOSURE, makeConstant(OBJ_VAL(functionNode)));

    for (int i = 0; i < functionNode->upvalueCount; i++) {
...
ObjFunction *compile(const char *source) {
    ...
    // пространство
    Scanner scannerInstance(source);
    ...
    // Зачищаем указатель на всякий случай
...
void markCompilerRoots() {
    Compiler *compilerNode = current;
    while (compilerNode != nullptr) {
        markObject(reinterpret_cast<Obj *>(compilerNode->function));
        compilerNode = compilerNode->enclosing;
    }
}
