...
VM vm; // Глобальный экземпляр виртуальной машины

// встроенная функция clock (возвращает время)
static Value clockNative(int argCount, Value *args) {
    return NUMBER_VAL(static_cast<double>(std::clock()) / CLOCKS_PER_SEC);
}
...
void VM::defineNative(const char *name, NativeFn function) {
    push(OBJ_VAL(copyString(name, static_cast<int>(std::strlen(name)))));
...
Value VM::peek(int distance) const { return stackTop[-1 - distance]; }
...
InterpretResult VM::interpretBinary(ObjFunction *function) {
    if (function == nullptr)
        return INTERPRET_COMPILE_ERROR;

    // кладем функцию на стек, чтобы сборщик мусора ее не удалил
    push(OBJ_VAL(function));
